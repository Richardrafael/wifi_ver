      ...
      if (!mysqli_stmt_num_rows($vert)) {
        foreach ($controller_urls as $controller_url) {
          ...
          if ($login) {
            ...
            $authorization_results[$controller_url] = false;
            $iserror = true;
            $error = 'Falha ao autorizar o dispositivo em uma controlado: ';
            //. $controller_url;
            // break;
            // exit();
          }
  ...
  if (isset($_SESSION['id']) && isset($_SESSION['ap'])) {
    ...
    switch ($tipo1) {
      ...
      case "me":
        ...
        $login_time = 720;
        $consulta = "SELECT * FROM dbamv.prestador 
        WHERE cd_tip_presta = 8 AND TP_SITUACAO = 'A'
         AND nr_cpf_cgc = :nr_cpf_cgc AND 
         ds_codigo_conselho = :ds_codigo_conselho 
         and NM_prestador = :nome OR 
         NM_MNEMONICO = :nome ";
        $stmt = oci_parse($conn_ora, $consulta);
        $nome = strtoupper($_POST['nome']);
        oci_bind_by_name($stmt, ':nome', $nome);
    ...
    if (oci_execute($stmt)) {
      if ($row = oci_fetch_assoc($stmt)) {
        ...
        if ($vert) {
          ...
          if (!mysqli_stmt_num_rows($vert)) {
            foreach ($controller_urls1 as $controller_url) {
              $unifi_connection = new UniFi_API\Client($controller_user, $controller_password, $controller_url, $site_id, $controller_version);
              $login = $unifi_connection->login();
            ...
            if (count($authorization_results) == count($controller_urls1) && !in_array(false, $authorization_results)) {
              switch ($tipo1) {
                case "fu":
    ...
    $iserror = true;
    $error = 'Vari&aacuteveis de sess�o ausentes ou inv�lidas.';
  }
}
$pagina = '';
...
<head>
  ...
  <title>Authoriza��o medico</title>
</head>
